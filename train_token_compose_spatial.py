...
from loss_utils_spatial import get_grounding_loss_by_layer, get_word_idx
...
###############################################################This part was added by Raphael Bernas############################################################################################################
                # spatial relation : left, right, top, bottom
                spatial_words_dict = {"left": ["left"], "right": ["right"], "top": ["top", "above", "upon"], "bottom": ["bottom", "under", "beneath", "on"]}
                ...
                for i in range(len(spatial_words_dict)):
                    spatial_words = spatial_words_dict[list(spatial_words_dict.keys())[i]]
                    for j in range(len(spatial_words)):
                        if spatial_words[j] in prompts[0].split()[:-1]:
                            parts = prompts[0].split(" "+spatial_words[j]+" ")
                            print(parts)
                            if len(parts) == 2:
                                # find the word before the spatial word and after the spatial word
                                words_before = parts[0]
                                words_after = parts[1]
                                indice_word_in_word_token_idx_ls = 0
                                for item in postprocess_seg_ls:
                                    words = item[0][0]
                                    words = words.lower()
                                    if words in words_before:
                                        words_indices = get_word_idx(prompts[0], words, tokenizer)
                                        spatial_results[list(spatial_words_dict.keys())[i] + "1"].append(indice_word_in_word_token_idx_ls)
                                    if words in words_after:
                                        words_indices = get_word_idx(prompts[0], words, tokenizer)
                                        spatial_results[list(spatial_words_dict.keys())[i] + "2"].append(indice_word_in_word_token_idx_ls)
                                    indice_word_in_word_token_idx_ls += 1
                                if len(spatial_results[list(spatial_words_dict.keys())[i] + "1"]) == 0 or len(spatial_results[list(spatial_words_dict.keys())[i] + "2"]) == 0 : # Case where it is the last word
                                    spatial_results[list(spatial_words_dict.keys())[i] + "1"] = []
                                    spatial_results[list(spatial_words_dict.keys())[i] + "2"] = []
...
###############################################################End of part added ############################################################################################################
                ...
                for train_layer in train_layers_ls:
                    ...
                    layer_spatial_loss = attn_loss_dict["spatial_loss"]

                    grounding_loss_dict[f"token/{train_layer}"] = layer_token_loss
                    grounding_loss_dict[f"pixel/{train_layer}"] = layer_pixel_loss
                    grounding_loss_dict[f"spatial/{train_layer}"] = layer_spatial_loss

                    token_loss += layer_token_loss
                    pixel_loss += layer_pixel_loss
                    spatial_loss += layer_spatial_loss

                grounding_loss = token_loss_scale * token_loss + pixel_loss_scale * pixel_loss + spatial_loss_scale * spatial_loss
...
if __name__ == "__main__":
    ...
    parser.add_argument("--spatial_loss_scale", type=float, required=True)
